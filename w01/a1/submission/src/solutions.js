...
function normalizeVariable(value) {
  value = value.trim(); //to remove all leading blank lines
  value = value.replace(/[\s.]+/g, '_'); //replaces all black spaces and dots with '_'
  return value.toUpperCase(); //make it all uppercase
}
...
function createIframe(src, height, allowFullScreen) {
  ...
  if (!isNaN(heightNum) && heightNum >= 1) {
    heightNum = Math.floor(heightNum);
    heightFrame = ' height="' + heightNum + '"';
  }

  let iframe = '<iframe src="' + src + '"' + heightFrame + '';
  if (allowFullScreen) {
...
function fixPostalCode(postalCode) {
  const postalCodeFormat = /^[^DFIOQUWZ]\d[^DFIOQU]\d[^DFIOQU]\d$/i;
  postalCode = postalCode.replace(/\s+/g, '').toUpperCase();

  if (!postalCodeFormat.test(postalCode) || postalCode.length !== 6) {
...
function toProvince(postalCode, useShortForm) {
  const provinceMap = {
    A: { name: 'Newfoundland and Labrador', short: 'NL' },
    B: { name: 'Nova Scotia', short: 'NS' },
    C: { name: 'Prince Edward Island', short: 'PE' },
    E: { name: 'New Brunswick', short: 'NB' },
    G: { name: 'Quebec', short: 'QC' },
    H: { name: 'Quebec', short: 'QC' },
    J: { name: 'Quebec', short: 'QC' },
    K: { name: 'Ontario', short: 'ON' },
    L: { name: 'Ontario', short: 'ON' },
    M: { name: 'Ontario', short: 'ON' },
    N: { name: 'Ontario', short: 'ON' },
    P: { name: 'Ontario', short: 'ON' },
    R: { name: 'Manitoba', short: 'MB' },
    S: { name: 'Saskatchewan', short: 'SK' },
    T: { name: 'Alberta', short: 'AB' },
    V: { name: 'British Columbia', short: 'BC' },
    X: { name: 'Northwest Territories and Nunavut', short: 'NT' },
    Y: { name: 'Yukon', short: 'YT' }
  };
  ...
  const provinceInfo = provinceMap[first];

  if (!provinceInfo) {
    return null;
  }

  return useShortForm ? provinceInfo.short : provinceInfo.name;
}
...
function normalizeCoord(value) {
  const part = value.split(',').map((part) => part.trim());

  let lat, long;
  if (part.length === 2) {
    lat = parseFloat(part[0]);
    long = parseFloat(part[1]);
  } else if (part.length === 3) {
    long = parseFloat(part[0]);
    lat = parseFloat(part[1]);
  }
...
function countForProvince(provinceCode, ...postalCodes) {
  ...
  const format = /^[A-Z]\d[A-Z] \d[A-Z]\d$/;
  for (const postalCode of postalCodes) {
    if (!format.test(postalCode) && typeof postalCode === 'string') {
      continue;
    } else if (typeof postalCode !== 'string') {
      throw new Error('not a string!');
    }

    if (toProvince(postalCode, true) === provinceCode) {
      validPostalCodes.push(postalCode);
    }
  }
  return validPostalCodes.length;
}
/*******************************************************************************
...
function generateLicenseLink(licenseCode, targetBlank) {
  ...
  if (licenseTextMap[licenseCode]) {
    licenseText = licenseTextMap[licenseCode];
    const formatted = licenseCode.slice(3).toLocaleLowerCase();
    licenseURL = `https://creativecommons.org/licenses/${formatted}/4.0/`;
  }

  let link = `<a href="${licenseURL}">${licenseText}</a>`;
  if (targetBlank) {
    link = `<a href="${licenseURL}" target="_blank">${licenseText}</a>`;
  }
...
function pureBool(value) {
  const trueValues = new Set(['yes', 'y', 'oui', 'o', 't', 'true', 'vrai', 'v', '1', true]);
  const falseValues = new Set(['no', 'n', 'non', 'f', 'false', 'faux', '0', false]);
  ...
  if (trueValues.has(normalized) || (!isNaN(normalized) && Number(normalized) > 0)) {
    return true;
  }

  if (falseValues.has(normalized) || (!isNaN(normalized) && Number(normalized) <= 0)) {
    return false;
  }
...
function buildUrl(query, order, count, license) {
  ...
  const validOrders = ['ascending', 'descending', 'asc', 'desc'];
